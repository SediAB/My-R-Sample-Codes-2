...
n <- 20
n
myvec <- c(heads=1, tails=0)
myvec
coinoutcome <- vector(length = n, mode = "integer")
for(i in 1:n){
  coinoutcome[i] <- sample(myvec, 1)
}
print(coinoutcome)
```
# NA and which command
```{r}
...
mean(x[which(x$Ozone >31 & x$Temp > 90),]$Solar.R)
mean(x[which(x$Month == 6),]$Temp)
max(x[which(x$Month == 5),]$Ozone, na.rm = TRUE)
?which
```
